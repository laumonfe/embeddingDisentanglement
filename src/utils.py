...
def load_distilbert_with_projection_finetuned(model_path):
    """
    Loads a finetuned DistilBertModel and attaches the projection layer using config and weights from model_path.
    Assumes config and weights are stored as text_encoder/config.json and text_encoder/text_projection.bin.
    """
    import os
    import json
    import torch
    ...
    # Load projection weights and remap keys if necessary
    proj_weights = torch.load(proj_weights_path, map_location="cpu")
    if "linear.weight" in proj_weights:
        proj_weights["weight"] = proj_weights.pop("linear.weight")
    if "linear.bias" in proj_weights:
        proj_weights["bias"] = proj_weights.pop("linear.bias")
...
def load_vision_with_projection(vision_model_dir):
    """
    ...
    # Load vision encoder
    vision_model = CLIPVisionModel.from_pretrained(vision_model_dir)
